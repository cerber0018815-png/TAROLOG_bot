...
async def start_session_core(chat_id: int, user_id: int, context: ContextTypes.DEFAULT_TYPE):
    if USE_AI_WELCOME:
        ...
        welcome = "Здравствуйте. Я — таролог, работающий с колодой Райдера—Уэйта. Я готов рассмотреть ваш вопрос через призму символов и архетипов.\n\n"
                  "Для того чтобы карты заговорили максимально ясно и адресно, пожалуйста, опишите вашу ситуацию или вопрос как можно подробнее. Контекст — это ключ, который помогает мне настроить интерпретацию именно на вашу жизненную мелодию. Чем больше деталей вы предоставите, тем глубже и точнее будет диалог между вами и картами.\n\n"
                  "Я вытяну три карты и проведу для вас полный разбор. Что вас волнует? "
